...
btnNew.onclick = () => {
    ...
    items.push({
        ...
        // exitDate: type.value === 'Saída' ? new Date().toLocaleDateString('pt-BR') : null
    })
...
function insertItem(item, index) {
    ...
    tr.innerHTML = `
        ...
        <td>${item.amount}</td>
        <td>${item.date}</td>
        <td>${item.exitDate ? item.exitDate : ''}</td>
...
const getitnsBD = () => JSON.parse(localStorage.getItem('db_items')) ?? [];
const setItensBD = () =>
